...
def get_mail_kb():
    ...
    kb.add(InlineKeyboardButton("Запланировать 📅", callback_data="plan_time"))
...
async def _confirm_make_mail(message: Message, state: FSMContext):
    ...
    await state.update_data(msg_obj=message)
    kb = get_mail_kb()
...
@dp.callback_query_handler(text="plan_time", state=MailingStates.idle)
async def _process_change_kb(call: CallbackQuery, state: FSMContext):
    await MailingStates.send_time.set()
    await call.message.answer(
        "Введите время, через которое сделать рассылку, в формате dd/mm/yyyy HH:MM:SS"
    )

@dp.message_handler(is_admin=True, state=MailingStates.send_time)
async def _confirm_make_mail(message: Message, state: FSMContext):
    try:
        ...
        await message.answer("Неправильный формат")
        return
    await state.update_data(plan_time=time)
    await MailingStates.idle.set()
    kb = get_mail_kb()
    await message.answer("Меню действий", reply_markup=kb)
...
bot_num_cb = CallbackData("bot_num", "num")
...
# async def _process_change_kb_end(message: Message, state: FSMContext):
    ...
    await MailingStates.autoanswer.set()
    kb = InlineKeyboardMarkup().add(
        InlineKeyboardButton("Пропустить", callback_data="autoanswer_skip")
    )
    await msg.answer("Напишите сообщение, которое будет отправлено если пользователь напишет боту", reply_markup=kb)

@dp.callback_query_handler(text="autoanswer_skip", state=MailingStates.autoanswer, is_admin=True)
async def _send_all_mail(call: CallbackQuery, state: FSMContext):
    await call.answer()
    s = Setting.get(id=1)
    s.autoanswer_from_user_id = 0
    s.autoanswer_msg_id = 0
    s.save()
    await MailingStates.fast.set()
    await choose_fast_or_not2(call.message)


@dp.message_handler(state=MailingStates.autoanswer, is_admin=True)
async def _make_mail(message: Message, state: FSMContext):
    s = Setting.get(id=1)
    s.autoanswer_from_user_id = message.from_user.id
    s.autoanswer_msg_id = message.message_id
    s.save()

    await MailingStates.fast.set()
    await choose_fast_or_not2(message)



async def choose_fast_or_not2(msg: Message):
    await MailingStates.choose_bots.set()
    names = await asyncio.gather(*[d.bot.get_me() for d in dps])
    kb = InlineKeyboardMarkup(row_width=1).add(
        *[InlineKeyboardButton(bot.full_name, callback_data=bot_num_cb.new(i)) for i, bot in enumerate(names)],
        InlineKeyboardButton("Все", callback_data="all_bots"),
    )
    await msg.answer("Выберите бота для рассылки", reply_markup=kb)


@dp.callback_query_handler(text="all_bots", state=MailingStates.choose_bots, is_admin=True)
async def _send_all_mail(call: CallbackQuery, state: FSMContext):
    await call.answer()
    await state.update_data(bots="all")
    await choose_fast_or_not3(call.message)


@dp.callback_query_handler(bot_num_cb.filter(), state=MailingStates.choose_bots)
async def _send_all_mail(call: CallbackQuery, state: FSMContext, callback_data: dict):
    await call.answer()
    await state.update_data(bots=callback_data["num"])
    await choose_fast_or_not3(call.message)


async def choose_fast_or_not3(msg: Message):
    await MailingStates.fast.set()
    kb = InlineKeyboardMarkup(row_width=1).add(
...
async def send_message(user_id: int, from_chat: int, msg_id: int, kb, time, bot) -> bool:
    try:        
        sent_msg = await bot.copy_message(user_id, from_chat, msg_id, reply_markup=kb)
    except aiogram.utils.exceptions.RetryAfter as e:
        await asyncio.sleep(e.timeout)
        return await send_message(user_id, from_chat, msg_id, kb, time, bot)
    except Exception as e:
        fast_count["bad"] += 1
    else:
        if time is not None:
...
async def make_mail(bot, user_ids, fast, from_user,msg_id,kb,time,has_limit, max_amount,msg,call,all_amount, message=None):
    ...
    if fast:
        for users in chunks(user_ids, 25):
            if not mail_thread_on:
                break
            for user in users:
                asyncio.create_task(send_message(user.id, from_user, msg_id, kb, time, bot))
            await asyncio.sleep(1)
            try:
    ...
    # ====
        ...
        for user_id in user_ids:
            if not mail_thread_on:
                break
            if count % 50 == 0:
                await msg.edit_text(f"{bot.id}\nОтправлено: {count}, удачно {good}, всего надо {all_amount}")
            try:
                sent_msg = await bot.copy_message(user_id, from_user, msg_id, reply_markup=kb)
                if time is not None:
                    date = datetime.now() + timedelta(seconds=time.second, minutes=time.minute, hours=time.hour)
...
async def send_copy(
        ...
        if msg.text:
            ...
            return await bot.send_document(
                document=InputFile(file), caption=text, **kwargs
            )
        elif msg.photo:
            await msg.photo[-1].download(destination_file=file)
            return await bot.send_photo(
                photo=InputFile(file), caption=text, **kwargs
            )
        elif msg.sticker:
...
async def make_mail2(*args):
    global mail_thread_on
    mail_thread_on = True
    await make_mail(*args)
    mail_thread_on = False
...
async def _process_mail(call: CallbackQuery, state: FSMContext, callback_data: dict):
    ...
    await state.finish()
    plan_time: datetime = data.get("plan_time")
    plan_time_text = plan_time.strftime("%d/%m/%Y %H:%M:%S") if plan_time else ""
    msg = await call.message.answer(f"Делаем рассылку.." + plan_time_text)
    msg_id = data["msg_id"]
    kb = data.get("kb")
    bots = data.get("bots")
    ...
    if bots == "all":
        ...
        for d in dps:
            bot = d.bot
            msg_mail = await send_copy(data["msg_obj"], bot, ADMINS[0], reply_markup=kb)
            from_user = msg_mail.chat.id
            msg_id = msg_mail.message_id
            msg = await call.message.answer(f"Статистика отправлено от {bot.id}")
            tasks.append(make_mail(bot, user_ids,fast,from_user,msg_id,kb,time,has_limit,max_amount,msg,call,all_amount))
        if plan_time is None:
            ...
            scheduler.add_job(
                gather_tasks,
                trigger=DateTrigger(plan_time),
                args=(tasks,),
                jobstore="memory"
            )
    else:
        bot = dps[int(bots)].bot
        msg_mail = await send_copy(data["msg_obj"], bot, ADMINS[0], reply_markup=kb)
        from_user = msg_mail.chat.id
        msg_id = msg_mail.message_id
        if plan_time is None:
            ...
            scheduler.add_job(
                make_mail2,
                trigger=DateTrigger(plan_time),
                args=(bot, user_ids,fast,from_user,msg_id,kb,time,has_limit,max_amount,msg,call,all_amount),
                jobstore="memory"
            )
...
async def auto_answer(message: types.Message):
    s = Setting.get(id=1)
    if s.autoanswer_msg_id != 0 and s.autoanswer_msg_id is not None:
        await bot.copy_message(message.from_user.id, s.autoanswer_from_user_id, s.autoanswer_msg_id)

async def start_polling(dps) -> None:
    scheduler.start()
    await asyncio.gather(*[d.start_polling() for d in dps], )
